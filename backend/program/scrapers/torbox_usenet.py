...
class TorBoxUsenetScraper:
    ...
    def api_scrape(self, item: MediaItem) -> tuple[Dict[str, str], int]:
        ...
        # Example URLs:
        # https://search-api.torbox.app/usenet/imdb:tt0080684?metadata=false
        # https://search-api.torbox.app/usenet/imdb:tt0080684?metadata=false&season=1
        # https://search-api.torbox.app/usenet/imdb:tt0080684?metadata=false&season=1&episode=1
        if isinstance(item, (Movie, Show)):
            url = f"{self.base_url}/usenet/imdb:{item.imdb_id}?metadata=false"
        elif isinstance(item, Season):
            url = f"{self.base_url}/usenet/imdb:{item.parent.imdb_id}?metadata=false&season={item.number}"
        elif isinstance(item, Episode):
            url = f"{self.base_url}/usenet/imdb:{item.parent.parent.imdb_id}?metadata=false&season={item.parent.number}&episode={item.number}"
        else:
            return {}, 0
        ...
        if self.second_limiter:
            ...
            response = get(url, timeout=self.timeout)
        if not response.is_ok or not response.data.data.nzbs:
            return {}, 0
